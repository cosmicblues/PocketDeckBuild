...
from PocketDeckBuild.model.models import TrainerTable
from PocketDeckBuild.schema.schemas_trainers import TrainerCreate, TrainerUpdate
...
class SqlTrainerCRUD(TrainerCRUD):
    ...
    def get_trainer(dbb: Session):
        return dbb.query(TrainerTable).all()

    @staticmethod
    def get_trainer_by_id(dbb: Session, id: int):
        return dbb.query(TrainerTable).filter(TrainerTable.id == id).first()

    @staticmethod
    def create_trainer(dbb: Session, trainer: TrainerCreate):
        db_trainer = TrainerTable(**trainer.dict())
        dbb.add(db_trainer)
        dbb.commit()
    ...
    @staticmethod
    def update_trainer(dbb: Session, trainer: TrainerUpdate):
        trainer_in_db = SqlTrainerCRUD.get_trainer_by_id(dbb, TrainerTable.id)
