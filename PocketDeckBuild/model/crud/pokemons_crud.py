...
from PocketDeckBuild.model.models import PokemonTable
from PocketDeckBuild.schema.schemas_pokemons import PokemonCreate, PokemonUpdate
...
class SqlPokemonCRUD(PokemonCRUD):
    ...
    @staticmethod
    def get_pokemons(dbb: Session):
        return dbb.query(PokemonTable).all()

    @staticmethod
    def get_pokemon_by_id(dbb: Session, id: int):
        return dbb.query(PokemonTable).filter(PokemonTable.id == id).first()

    @staticmethod
    def create_pokemon(dbb: Session, pokemon: PokemonCreate):
        db_pokemon = PokemonTable(**pokemon.dict())
        dbb.add(db_pokemon)
        dbb.commit()
    ...
    @staticmethod
    def update_pokemon(dbb: Session, pokemon: PokemonUpdate):
        pokemon_in_db = SqlPokemonCRUD.get_pokemon_by_id(dbb, PokemonTable.id)
